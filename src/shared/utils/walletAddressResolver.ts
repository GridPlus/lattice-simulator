 ...
 */
export function resolveWalletAddresses(startPath: WalletPath, count: number): AddressInfo[] {
  ...
  if (!walletStore.isInitialized) {
    console.warn(
      '[WalletAddressResolver] Wallet store not initialized, falling back to empty array',
    )
    return []
  }
  ...
  if (addresses.length < count) {
    console.warn(
      `[WalletAddressResolver] Only found ${addresses.length} addresses, requested ${count}. You may need to create more accounts.`,
    )
  }
