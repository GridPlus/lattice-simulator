...
export function createBitcoinAccountFromHDKey(
  ...
  switch (addressType) {
    case 'legacy':
      ...
      payment = bitcoin.payments.p2pkh({ pubkey: Buffer.from(keyPair.publicKey!), network: btcNetwork })
      break
      
    case 'segwit':
      // P2WPKH (Pay-to-Witness-PubkeyHash) - native segwit, addresses start with 'bc1'
      payment = bitcoin.payments.p2wpkh({ pubkey: Buffer.from(keyPair.publicKey!), network: btcNetwork })
      break
      
    case 'wrapped-segwit':
      // P2SH-P2WPKH (Pay-to-Script-Hash wrapping P2WPKH) - addresses start with '3'
      const p2wpkh = bitcoin.payments.p2wpkh({ pubkey: Buffer.from(keyPair.publicKey!), network: btcNetwork })
