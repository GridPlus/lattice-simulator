...
import { BITCOIN_NETWORKS } from '../wallets/bitcoin'
import { getWalletConfig } from '../wallets/config'
import type { SignResponse } from '../types/device'
import type {
  WalletAccount,
...
export class SignatureEngine {
  ...
  async signData(
    ...
    console.log('[SignatureEngine] signData curve check:', {
      curve: request.curve,
    ...
    if (process.env.DEBUG_SIGNING === '1') {
      ...
      console.log('[SignatureEngine] signData request', debugInfo)
      try {
      } catch {
        // ignore
      }
    ...
    if (request.curve === EXTERNAL.SIGNING.CURVES.ED25519) {
      console.log('[SignatureEngine] Routing to Ed25519 handler')
      return this.signEd25519(request)
    }

    console.log('[SignatureEngine] NOT routing to Ed25519, continuing to coin-based logic')
    ...
    console.log('[SignatureEngine] Request payload info:', {
    ...
    console.log('[SignatureEngine] Request keys:', Object.keys(request))
  ...
  private async signEthereum(
    ...
    console.log('[SignatureEngine] Active Ethereum account:', {
      address: wallet.address,
      derivationPath,
    })
    ...
    if (request.schema === SIGNING_SCHEMA.ETH_MSG) {
      ...
      if (protocol === 'eip712') {
        ...
        if (request.isPrehashed) {
          ...
          if (typedDataBuffer.length > 32) {
            console.warn('[SignatureEngine] Truncating prehashed EIP-712 payload to 32 bytes', {
        ...
        console.log('[SignatureEngine] EIP-712 branch', {
          isPrehashed: request.isPrehashed ?? false,
      ...
      if (request.hashType === EXTERNAL.SIGNING.HASHES.NONE) {
        ...
        if (txData.length >= 32) {
          ...
          if (allZeros && first32.some(byte => byte !== 0)) {
            isPrehashed = true
            console.log('[SignatureEngine] Detected prehashed transaction')
          }
        }
        ...
        if (isPrehashed) {
          ...
          console.log('[SignatureEngine] Using prehashed transaction data directly')
        } else {
      ...
      try {
        ...
        if (candidateUncompressed !== publicKeyUncompressed) {
          console.warn(
            '[SignatureEngine] Recovered pubkey does not match derived pubkey; using derived key instead',
          )
        }
      } catch (err) {
        console.warn('[SignatureEngine] Failed to recover pubkey from signature', err)
      }
  ...
  private async signBLS12381(request: SigningRequest): Promise<SignatureResult> {
    ...
    console.log('[SignatureEngine] BLS12-381 signature components:', {
      publicKeyLength: publicKeyBuffer.length,
      signatureLength: signatureBuffer.length,
      path: request.path,
    })
      ...
      for (const [typeName, entries] of Object.entries(value.types)) {
        ...
        for (const entry of entries) {
          try {
            ...
            console.warn('[SignatureEngine] Skipping invalid EIP-712 type entry', {
              typeName,
              entry,
              detail,
            })
  ...
  private coerceScalarValue(type: string, value: any): any {
    ...
    if (baseType.startsWith('uint') || baseType.startsWith('int')) {
      ...
      if (process.env.DEBUG_SIGNING === '1') {
        console.log('[SignatureEngine] coerceScalarValue bits', {
          type: baseType,
          parsedBits: bitsRaw,
          bitWidth,
        })
      ...
      if (baseType.startsWith('uint')) {
        ...
        if (process.env.DEBUG_SIGNING === '1') {
          console.log('[SignatureEngine] coerceScalarValue uint', {
            type: baseType,
      ...
      if (process.env.DEBUG_SIGNING === '1') {
        console.log('[SignatureEngine] coerceScalarValue int', {
          type: baseType,
          originalDecimal: bigintValue.toString(),
          normalizedDecimal: normalized.toString(),
          originalHex: this.formatBigIntForLogging(bigintValue),
  ...
  private decodeEip712Payload(payload: Buffer): any {
    try {
      ...
      const detail = error instanceof Error ? error.message : String(error)
      console.error('[SignatureEngine] Failed to decode EIP-712 payload', detail)
  ...
  private findWalletAccount(
    ...
    console.log(
      `[SignatureEngine] Finding wallet account for path: ${path.join('/')}, walletAccounts.size: ${walletAccounts.size}`,
    )
