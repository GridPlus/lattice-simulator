...
describe('parseProtocolMessage', () => {
  ...
  function createMessageBuffer(
    ...
    buffer.writeUInt8(messageType, offset); offset += 1
    messageId.copy(buffer, offset); offset += 4
    buffer.writeUInt16BE(payloadLength, offset); offset += 2
    
    // Write request type
    buffer.writeUInt8(requestType, offset); offset += 1
    
    // Write payload
    payload.copy(buffer, offset); offset += payloadLength
  ...
  describe('Payload length handling', () => {
    ...
    it('should handle zero payload length', () => {
      const buffer = createMessageBuffer(0x01, 0x02, Buffer.from([0x01, 0x02, 0x03, 0x04]), 0x01, Buffer.alloc(0))
    ...
    it('should handle maximum payload length', () => {
      ...
      const buffer = createMessageBuffer(0x01, 0x02, Buffer.from([0x01, 0x02, 0x03, 0x04]), 0x01, maxPayload)
  ...
  describe('Request type handling', () => {
    it('should correctly extract request type byte', () => {
      ...
      const buffer = createMessageBuffer(0x01, 0x02, Buffer.from([0x01, 0x02, 0x03, 0x04]), requestType)
  ...
  describe('Connect request handling (requestType = 0x01)', () => {
    it('should correctly parse connect request with 65-byte public key', () => {
      ...
      const buffer = createMessageBuffer(0x01, 0x02, Buffer.from([0x01, 0x02, 0x03, 0x04]), 0x01, publicKey)
    ...
    it('should reject connect request with wrong payload size', () => {
      ...
      const buffer = createMessageBuffer(0x01, 0x02, Buffer.from([0x01, 0x02, 0x03, 0x04]), 0x01, wrongSizePayload)
  ...
  describe('Encrypted request handling (requestType != 0x01)', () => {
    it('should correctly parse encrypted request with ephemeral ID', () => {
      ...
      const buffer = createEncryptedMessageBuffer({ requestType: 0x02, ephemeralId, encryptedData, checksum: 0x87654321, messageId: Buffer.from([0x01, 0x02, 0x03, 0x04]) })
    ...
    it('should reject encrypted request with insufficient payload size', () => {
      ...
      const buffer = createMessageBuffer(0x01, 0x02, Buffer.from([0x01, 0x02, 0x03, 0x04]), 0x02, smallPayload)
  ...
  describe('Checksum handling', () => {
    it('should correctly read 4-byte checksum (big endian)', () => {
      ...
      const buffer = createMessageBuffer(0x01, 0x02, Buffer.from([0x01, 0x02, 0x03, 0x04]), 0x01, Buffer.alloc(65), checksum)
  ...
  describe('Message size validation', () => {
    it('should validate that parsed size matches buffer size', () => {
      const buffer = createMessageBuffer(0x01, 0x02, Buffer.from([0x01, 0x02, 0x03, 0x04]), 0x01, Buffer.alloc(65))
  ...
  describe('Edge cases and error conditions', () => {
    ...
    it('should handle large payload sizes', () => {
      ...
      const buffer = createMessageBuffer(0x01, 0x02, Buffer.from([0x01, 0x02, 0x03, 0x04]), 0x02, largePayload)
      
      const payloadLength = buffer.readUInt16BE(6)
      expect(payloadLength).toBe(1000)
      
      // Verify payload content
      let offset = 8 + 1 // header + requestType
    ...
    it('should handle zero-length encrypted payload', () => {
      ...
      const buffer = createMessageBuffer(0x01, 0x02, Buffer.from([0x01, 0x02, 0x03, 0x04]), 0x02, emptyPayload)
  ...
  describe('Integration scenarios', () => {
    ...
    it('should parse a complete encrypted request message', () => {
      ...
      const buffer = createEncryptedMessageBuffer({ requestType: 0x04, ephemeralId, encryptedData, checksum, messageId })
