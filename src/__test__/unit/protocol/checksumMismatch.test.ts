...
describe('Checksum Mismatch Reproduction', () => {
  ...
  it('should reproduce checksum mismatch with corrupted encrypted data', async () => {
    ...
    const sharedSecret = Buffer.from('e28d7864b86059adeec8b67a3ceeb3f11092126a0c60fcad3ab0066fd168d07f', 'hex')
  ...
  it('should simulate client-side checksum validation failure', async () => {
    ...
    if (checksumMismatch) {
      const error = new Error(`Checksum mismatch in decrypted Lattice data, respData.checksum=${receivedChecksum}, validChecksum=${expectedChecksum}`)
