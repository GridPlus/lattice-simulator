...
const MULTI_CLIENT_PAIRING_TIMEOUT_MS = 20000
...
describe('Multi-client pairing support', () => {
  it(
    'allows multiple clients to pair sequentially and re-pair returns already',
    async () => {
    ...
    MULTI_CLIENT_PAIRING_TIMEOUT_MS,
  )

  it(
    'connect reports pairing status per client and unpairAll resets',
    async () => {
      const simulator = new DeviceSimulator({
        deviceId: 'test-device',
        pairingCode: pairingSecret,
  ...
  it(
    'decrypts secure requests using the correct session secret',
    async () => {
      const simulator = new DeviceSimulator({
        deviceId: 'test-device',
        pairingCode: pairingSecret,
